...
:::warning フォルダ名・プロジェクト名に日本語などの2バイト文字を使わない
Unity プロジェクトのフォルダ名やプロジェクト名に、日本語・全角文字などの2バイト文字を含めると、Unity / VCC / SDK / 一部ツールで読み込みやビルド、パッケージ導入時に問題が発生する場合があります。
プロジェクト名と保存先フォルダ名は、半角英数字のみ（例: `LuminousOasis_Project`）で作成してください。
:::
